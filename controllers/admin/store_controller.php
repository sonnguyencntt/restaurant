...
class StoreController extends BaseController
{
    ...
    {
        $data = $this->storeModal->selectAllData();
        $this->render('store.index', array('title_content' => 'Area', 'data' => $data,'userGroup' => $this->userGroup), 'layouts.application');
    }
    ...
    public function insert()
    {
        ...
        if (Myfunction::validateRegrex([$name, $active])) {
            $result = $this->storeModal->insert($name, $active, $datenow);
            if (!$result) {
    ...
    public function delete()
    {
        ...
        $result = $this->storeModal->delete($id_store);
        if (!$result) {
    ...
    {
        $id_store = filter_input(INPUT_POST, 'id_store', FILTER_SANITIZE_STRING);
        $result = $this->storeModal->edit($id_store);
        if (count($result) > 0) {
    ...
    public function update()
    {
        ...
        $result = $this->storeModal->update($id_store, $name, $active);
        $active = '<span class="label label-success">' . $active . '</span>';
